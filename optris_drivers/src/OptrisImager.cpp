...
{
  if (!_imager.init(&params, dev->getFrequency(), dev->getWidth(), dev->getHeight())) {
    std::cerr << "Imager init failed" << std::endl;
    return;
  }
...
{
  std::cout << "[optris] Start streaming" << std::endl;
  _dev->startStreaming();

  ros::NodeHandle n;  
  float maxFramerate = _imager.getMaxFramerate();
  std::cout << "[optris] Imager max framerate: " << maxFramerate << "." << std::endl;
  ros::Timer timer = n.createTimer(ros::Duration(1.0/maxFramerate), &OptrisImager::onTimer, this);
  ros::spin();
